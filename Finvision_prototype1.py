nse_companies# 1. User Searches name of company using its NSE initials
    #import the .txt file created for the same.
# find the Company corresponding to NSE code in the above string labelled "str"
# Create a dictionary of NSE companies
nse_companies = {
    "INFY": "Infosys",
...
def get_financial_data(company_name):
    """Get financial data for the company using Yahoo Finance"""
    ticker = yf.Ticker(company_name)
    data = ticker.info
    return data

def analyze_market_sentiment(company_name):
    """Analyze market sentiment for the company using news reports"""
    articles = []
    for article in newspaper.search(company_name):
...
def main():
    nse_code = input("Enter the NSE code: ")
    company_name = nse_companies.get(nse_code.upper())
    if company_name:
        print(f"Company Name: {company_name}")
        data = get_financial_data(company_name)
        print("Financial Data:")
        print(data)
        sentiment = analyze_market_sentiment(company_name)
...
# Retrieve financial data using Yahoo Finance
ticker = yf.Ticker(company_name)
...
# Retrieve news articles using Newspaper3k
articles = newspaper.search(company_name)
...
# 3. constructs .csv file using amassed financial data.
...
# assume you have a function to retrieve financial data for the company
def get_financial_data(company_name):
...
# retrieve financial data for the company
financial_data = get_financial_data(company_name)

# create a CSV writer
with open('company_financial_data.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)

    # write the header row
    writer.writerow(['Company Name'] + list(financial_data.keys()))

    # write the data row
    writer.writerow([company_name] + list(financial_data.values()))

with open('company_financial_data.csv', 'a', newline='') as csvfile:
    writer = csv.writer(csvfile)
...
with open('input.csv', 'r') as input_file:
    ...
    with open('output.csv', 'w', newline='') as output_file:
        ...
        for row in reader:
            # Remove unwanted characters and undefined values
            cleaned_row = [re.sub(r'[#$]', '', cell) if cell != 'undefined' else '' for cell in row]

            # Write the cleaned row to the output CSV file
            writer.writerow(cleaned_row)


# 7. Visualising data through Scatter plot, plotted against data from predefined data set, also input, by NIFTY500 from download link provided to code.

import pandas as pd
import matplotlib.pyplot as plt

# Load the predefined data set from the NIFTY500 download link
nifty500_data = pd.read_csv('nifty500_data.csv')

# Load the data from the BBundefined file
bb_data = pd.read_csv('BBundefined.csv', na_values=['undefined', '$#$'], skiprows=1)

# Clean the data by replacing unwanted characters and undefined values
bb_data = bb_data.replace({r'[#$]': ''}, regex=True).fillna('')

# Create a scatter plot of the cleaned data against the NIFTY500 data
plt.scatter(bb_data['column1'], nifty500_data['column2'])
plt.xlabel('BB Data')
plt.ylabel('NIFTY500 Data')
plt.title('Scatter Plot of BB Data vs NIFTY500 Data')
plt.show()

# 8. The system will then use this data to predict future stock prices of the company using a machine learning model.
...
new_data = pd.get_dummies(new_data, columns=['Open', 'High', 'Low'])  # one-hot encoding
...
# Preprocess the data
df.dropna(inplace=True)  # handle missing values
df['Close'] = df['Close'].diff().dropna()  # make the time series stationary
...
# 9. Constructing Regression Line and finding Beta-Value of Stock
...
X = df['Market Return'].values.reshape(-1, 1)  # market returns
y = df['Return'].values  # stock returns
model = LinearRegression()
model.fit(X, y)

# Find the beta-value of the stock
beta = model.coef_[0]

# Classify the stock
if beta < 1:
    print('Stable Investment')
elif beta > 1:
    print('Risky Investment')
else:
    print('Average Stock')
...
# Calculate the CAGR of the company
...
if cagr > df['Market Return'].mean():
    if df['Return'].std() < 1:
        print('Very Good Investment')
    else:
        print('Good Investment')
elif cagr < df['Market Return'].mean():
    print('Bad Investment')
else:
    print('Wait before making a stock sale')
